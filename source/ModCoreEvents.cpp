...
void ModCoreEvents::OnReadFile(const HRZR::String& CorePath, const HRZR::Array<HRZR::Ref<HRZR::RTTIRefObject>>& Objects)
{
	...
	for (auto& object : Objects)
	{
...
#if 0
		const static std::vector<const HRZR::RTTI *> scanForTypes = {
			HRZR::RTTI::FindTypeByName("EntityResource"),
			HRZR::RTTI::FindTypeByName("ViewDependentEntityResource"),
			HRZR::RTTI::FindTypeByName("InventoryEntityResource"),
			HRZR::RTTI::FindTypeByName("InventoryPlaceEntityAbilityResource"),
			HRZR::RTTI::FindTypeByName("InventoryReviveDroneAbilityResource"),
			HRZR::RTTI::FindTypeByName("InventoryWeaponResource"),
		...
		for (auto type : scanForTypes)
		{
			if (rtti->IsExactKindOf(type))
			{
				auto& components = object->GetMemberRefUnsafe<HRZR::Array<HRZR::Ref<HRZR::RTTIRefObject>>>("EntityComponentResources");
				bool done = false;

				for (auto& component : components)
				{
					//spdlog::info("Component: {}", component->GetRTTI()->GetSymbolName());

					const static auto ItemDescriptionComponentResourceRTTI = HRZR::RTTI::FindTypeByName("ItemDescriptionComponentResource");

					if (component->GetRTTI()->IsKindOf(ItemDescriptionComponentResourceRTTI))
					{
						auto& localizedName = component->GetMemberRefUnsafe<HRZR::Ref<HRZR::RTTIRefObject>>("LocalizedItemName");

						if (localizedName)
						{
							const char *namePart = *reinterpret_cast<const char **>(reinterpret_cast<uintptr_t>(localizedName.GetPtr()) + 0x20);
							uint16_t lengthPart = *reinterpret_cast<uint16_t *>(reinterpret_cast<uintptr_t>(localizedName.GetPtr()) + 0x28);

							std::string_view nameView(namePart, lengthPart);

							spdlog::info("    # [ \"{}\", \"{}\", \"{}\" ] # {}", object->m_UUID, CorePath, nameView, rtti->GetSymbolName());
				...
				if (!done)
				{
					auto& setupName = object->GetMemberRefUnsafe<HRZR::String>("Name");
					spdlog::info("    # [ \"{}\", \"{}\", \"{}\" ] # {}", object->m_UUID, CorePath, setupName, rtti->GetSymbolName());
				}
		...
		// Log object names
		const static auto customRTTI = HRZR::RTTI::FindTypeByName("CameraMagnetParmResource");

		if (rtti->IsKindOf(customRTTI))
		{
			//spdlog::info("{}", rtti->AsCompound()->SerializeObject(object.GetPtr(), 3).value());
			spdlog::info("{},", object->m_UUID);
		}
#endif
